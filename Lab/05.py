...
def firstN(I,n):
    return [I.__next__() for i in range(n)]
...
def makeDDimensionalArray(d,n,f,nth=0):
    if d==1:
        return [f([x+nth]) for x in range(n)] 
    else:       
      return [makeDDimensionalArray(d-1,n,f,x) for x in range(n) if d > 1 else [f([x+nth]) for x in range(n)]]
    #REMOVE THE PASS AND WRITE YOUR CODE HERE
